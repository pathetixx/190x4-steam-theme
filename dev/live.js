...
(function () {
  ...
  // клик по родной (скрытой) кнопке нижнего бара по тексту
  function clickBottom(label) {
    var bb = document.querySelector("._3vCzSrrXZzZjVJFZNg9SGu") ||
             document.querySelector('[class*="bottombar_BottomBarContainer_"]');
    if (!bb) return false;
    var els = [].slice.call(bb.querySelectorAll("*"));
    for (var i = 0; i < els.length; i++) {
      if ((els[i].textContent || "").trim().indexOf(label) === 0) {
        var e = els[i];
        while (e && e !== bb) {
          if (e.onclick || e.getAttribute("role") === "button" || /Button|Btn/.test(e.className)) { e.click(); return true; }
          e = e.parentElement;
        }
  ...
  var ICON = {
    ...
    add: '<rect x="4" y="4" width="16" height="16" rx="2"/><path d="M12 8v8M8 12h8"/>',
